...
class SpeechRecognitionRepositoryImpl @Inject constructor(
        ...
        val responseObserver = object : ApiStreamObserver<StreamingRecognizeResponse> {
            override fun onNext(response: StreamingRecognizeResponse?) {
                response?.resultsList?.forEach { result ->
                    ...
                    if (transcript != null) {
                        trySend(SpeechRecognitionRepository.TranscriptionResult(transcript, speakerTag, result.isFinal))
                    }
                }
            }
    ...
    override fun transcribeAudioFile(audioBytes: ByteArray): Flow<SpeechRecognitionRepository.TranscriptionResult> = callbackFlow {
        val client = getSpeechClient()
        ...
        try {
            ...
            response.resultsList.forEach { result ->
                ...
                val transcript = alternative?.transcript
                if (transcript != null) {
                    trySend(SpeechRecognitionRepository.TranscriptionResult(transcript, isFinal = true))
                }
            }
            close()
        } catch (e: Exception) {
